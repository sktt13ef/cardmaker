...
const Neumorphism: React.FC<NeumorphismProps> = ({ data, scale, progressBarConfig }) => {
  ...
  const renderProgressBar = (position: 'top' | 'bottom') => {
    ...
    return (
      <div style={{
        ...
        background: '#f0f2f5',
        borderBottom: position === 'top' ? '1px solid #d1d5db' : undefined,
        borderTop: position === 'bottom' ? '1px solid #d1d5db' : undefined,
      }}>
        <div style={{ width: '100%', height: 4, background: '#d1d5db', borderRadius: 2, display: 'flex', position: 'relative' }}>
          <div style={{
            position: 'absolute', left: 0, top: 0, height: '100%',
            width: `${((activeIndex + 1) / segmentCount) * 100}%`,
            background: '#4a5568', borderRadius: 2, transition: 'width 0.3s ease',
          }} />
          {Array.from({ length: segmentCount - 1 }, (_, i) => (
            <div key={i} style={{
              position: 'absolute', left: `${((i + 1) / segmentCount) * 100}%`,
              top: 0, width: 2, height: '100%', background: '#f0f2f5', transform: 'translateX(-50%)',
            }} />
          ))}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          {segmentLabels.slice(0, segmentCount).map((label, index) => (
            <div key={index} style={{
              ...
              color: index <= activeIndex ? '#4a5568' : '#9ca3af', transition: 'color 0.3s ease',
  ...
  return (
    <div style={{ width: 1920, height: 1080, transform: `scale(${scale})`, transformOrigin: 'top left', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="neu-container relative box-border w-full h-full overflow-hidden flex flex-col items-center justify-center" style={{ flex: 1 }}>
        <div
          ref={wrapperRef}
...
export const neumorphismTemplate: TemplateConfig = {
  ...
  icon: 'contrast',
  render: (data, scale, progressBarConfig) => React.createElement(Neumorphism, { data, scale, progressBarConfig }),
