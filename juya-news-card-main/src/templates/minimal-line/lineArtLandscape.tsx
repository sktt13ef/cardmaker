...
const LineArtLandscape: React.FC<LineArtLandscapeProps> = ({ data, scale, progressBarConfig }) => {
  ...
  const renderProgressBar = (position: 'top' | 'bottom') => {
    ...
    return (
      <div style={{ width: '100%', padding: '16px 48px', background: '#f8fafb' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          {config.segmentLabels.slice(0, config.segmentCount).map((label, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 24, height: 4, borderRadius: 2, background: index <= config.activeIndex ? '#6b8fa3' : '#e0e0e0', opacity: index <= config.activeIndex ? 0.8 : 0.3 }} />
  ...
  return (
    <div style={{ width: 1920, height: 1080, transform: 'scale(' + scale + ')', transformOrigin: 'top left', overflow: 'hidden', display: 'flex', flexDirection: 'column', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Noto Sans SC", Roboto, sans-serif', background: '#f8fafb' }}>
      ...
      <div ref={wrapperRef} style={{ flex: 1, padding: innerPadding, display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(' + cols + ', 1fr)', gap: gridGap, alignItems: 'start' }}>
          {data.cards.slice(0, N).map((card, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '14px', padding: cardPadding, background: '#FFFFFF', borderRadius: 4, boxShadow: '0 1px 4px rgba(107,143,163,0.06)', border: '1px solid rgba(107,143,163,0.12)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{ flex: 1, height: '1px', background: 'rgba(107,143,163,0.15)' }} />
              </div>
