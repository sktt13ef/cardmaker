...
interface LineGridCardProps {
  data: GeneratedContent;
  scale: number;
  progressBarConfig?: ProgressBarConfig;
}

const LineGridCard: React.FC<LineGridCardProps> = ({ data, scale, progressBarConfig }) => {
  ...
  const numberSize = isSingleCard ? '140px' : '72px';
  const numberFontSize = isSingleCard ? '64px' : '32px';
  const titleSize = isSingleCard ? '80px' : '40px';
  const descSize = isSingleCard ? '48px' : '26px';
  const cardPadding = isSingleCard ? '60px' : '40px';
...
export const lineGridCardTemplate: TemplateConfig = {
  id: 'lineGridCard',
  name: '线条网格',
  description: '线条网格装饰，清晰分割，现代简洁',
  icon: 'grid_4x4',
  render: (data, scale, progressBarConfig) => React.createElement(LineGridCard, { data, scale, progressBarConfig }),
  generateHtml: (data) => generateDownloadableHtml(data, 'lineGridCard'),
};
