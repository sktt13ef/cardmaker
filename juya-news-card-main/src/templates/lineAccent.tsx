...
const LineAccent: React.FC<LineAccentProps> = ({ data, scale, progressBarConfig }) => {
  ...
  const numberSize = isSingleCard ? '100px' : '60px';
  const numberFontSize = isSingleCard ? '48px' : '28px';
  const titleSize = isSingleCard ? '72px' : '40px';
  const descSize = isSingleCard ? '42px' : '26px';
          ...
          {segmentLabels.slice(0, segmentCount).map((label, index) => (
            <div
              ...
              style={{
                ...
                fontWeight: 500,
                color: index <= activeIndex ? '#2563eb' : '#9ca3af',
        ...
        <div style={{ 
          ...
          {data.cards.slice(0, N).map((card, i) => (
            <div key={i} style={{ 
              ...
              gap: isSingleCard ? '50px' : '20px',
              padding: isSingleCard ? '60px' : '40px',
              background: '#f8f8f8',
              borderRadius: isSingleCard ? '24px' : '16px',
              position: 'relative',
              borderLeft: '8px solid #2563eb',
              alignItems: isSingleCard ? 'flex-start' : 'stretch'
            }}>
              {/* Number Badge - 改为左侧独立区域 */}
              <div style={{
                width: numberSize,
                height: numberSize,
                flexShrink: 0,
              ...
              {/* Content */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: isSingleCard ? '24px' : '16px', flex: 1 }}>
                {/* Title */}
                <h3 style={{
                  fontSize: titleSize,
                  fontWeight: 700,
...
export const lineAccentTemplate: TemplateConfig = {
  id: 'lineAccent',
  name: '线条强调',
  description: '侧边线条强调，垂直分割，时尚现代',
  icon: 'vertical_align_center',
  render: (data, scale, progressBarConfig) => React.createElement(LineAccent, { data, scale, progressBarConfig }),
  generateHtml: (data) => generateDownloadableHtml(data, 'lineAccent'),
};
