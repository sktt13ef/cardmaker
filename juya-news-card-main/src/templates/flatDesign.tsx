...
const FlatDesign: React.FC<FlatDesignProps> = ({ data, scale, progressBarConfig }) => {
      ...
      <div style={{
        ...
        <div style={{ width: '100%', height: 4, background: '#e5e7eb', borderRadius: 2, display: 'flex', position: 'relative' }}>
          <div style={{
            ...
            background: '#1e293b', borderRadius: 2, transition: 'width 0.3s ease',
          }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          {segmentLabels.slice(0, segmentCount).map((label, index) => (
            <div key={index} style={{
              ...
              color: index <= activeIndex ? '#1e293b' : '#9ca3af', transition: 'color 0.3s ease',
  ...
  return (
    <div style={{ width: 1920, height: 1080, transform: `scale(${scale})`, transformOrigin: 'top left', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="flat-container relative box-border w-full h-full overflow-hidden flex flex-col items-center justify-center" style={{ flex: 1 }}>
        <div
          ref={wrapperRef}
...
export const flatDesignTemplate: TemplateConfig = {
  ...
  icon: 'rectangle',
  render: (data, scale, progressBarConfig) => React.createElement(FlatDesign, { data, scale, progressBarConfig }),
