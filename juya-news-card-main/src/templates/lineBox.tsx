...
const LineBox: React.FC<LineBoxProps> = ({ data, scale, progressBarConfig }) => {
  ...
  const numberSize = isSingleCard ? '120px' : '64px';
  const numberFontSize = isSingleCard ? '56px' : '28px';
        ...
        <div style={{ 
          ...
          {data.cards.slice(0, N).map((card, i) => (
            <div key={i} style={{ 
              ...
              padding: isSingleCard ? '60px 50px' : '60px 40px 40px 40px',
              border: '4px solid #000',
              borderRadius: isSingleCard ? '24px' : '20px',
              position: 'relative',
              background: '#fff',
              alignItems: isSingleCard ? 'center' : 'center',
              textAlign: isSingleCard ? 'left' : 'center'
            }}>
              {/* Number Badge */}
              <div style={{
                position: isSingleCard ? 'relative' : 'absolute',
                top: isSingleCard ? 'auto' : '-32px',
                left: isSingleCard ? 'auto' : '50%',
                transform: isSingleCard ? 'none' : 'translateX(-50%)',
                width: numberSize,
                height: numberSize,
                display: 'flex',
                alignItems: 'center',
              ...
              {/* Content */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', flex: 1, textAlign: isSingleCard ? 'left' : 'center' }}>
                {/* Title */}
                <h3 style={{
                  fontSize: titleSize,
                  fontWeight: 700,
...
export const lineBoxTemplate: TemplateConfig = {
  id: 'lineBox',
  name: '线条盒',
  description: '线框盒子装饰，卡片式布局，简洁整齐',
  icon: 'check_box_outline_blank',
  render: (data, scale, progressBarConfig) => React.createElement(LineBox, { data, scale, progressBarConfig }),
  generateHtml: (data) => generateDownloadableHtml(data, 'lineBox'),
};
